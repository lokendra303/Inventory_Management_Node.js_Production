...
const SalesInvoices = () => {
  ...
  const handleDownloadPDF = async (invoiceId, invoiceNumber) => {
    try {
      setLoading(true);
      const response = await apiService.get(`/sales-invoices/${invoiceId}/pdf?download=true`, {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `SI_${invoiceNumber}.pdf`);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      setLoading(true);
      const response = await apiService.post(`/sales-invoices/${selectedInvoiceForEmail.id}/email`, {
        email: emailAddress
      });
  ...
  return (
    <div style={{ padding: '16px' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}><FileTextOutlined /> Sales Invoices</Title>
        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreateInvoice}>Create Invoice</Button>
      </div>
      <Card>
        <Space style={{ marginBottom: 16, flexWrap: 'wrap' }}>
          <Input placeholder="Search invoice or customer..." prefix={<SearchOutlined />} value={searchText} onChange={e => setSearchText(e.target.value)} style={{ width: '100%', maxWidth: 240 }} allowClear />
      ...
      <Modal title={`${modalMode === 'create' ? 'Create' : 'Edit'} Sales Invoice`} open={modalVisible} onCancel={() => setModalVisible(false)} width="min(1200px, 96vw)" style={{ top: 16 }} footer={null} destroyOnClose>
        <InvoiceForm type="sales" invoiceId={selectedInvoiceId} onSave={handleModalSave} />
      </Modal>
      <Modal title="Email Invoice" open={emailModalVisible} onCancel={() => setEmailModalVisible(false)} onOk={handleSendEmail} okText="Send Email" confirmLoading={loading}>
