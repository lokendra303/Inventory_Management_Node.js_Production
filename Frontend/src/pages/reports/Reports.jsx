...
const Reports = () => {
  ...
  const fetchPurchaseReport = async () => {
    try {
      ...
      if (response.success) {
        ...
        console.log('Purchase report failed:', response.error);
        setPurchaseData([]);
      ...
      setPurchaseData([]);
      if (error.code !== 'ECONNABORTED') {
        message.error('Failed to fetch purchase report: ' + (error.message || 'Unknown error'));
      }
    } finally {
      setLoading(false);
    }
  ...
  const fetchAdjustmentReport = async () => {
    try {
      ...
      console.error('Adjustment report error:', error);
      setAdjustmentData([]);
      if (error.code !== 'ECONNABORTED') {
  ...
  useEffect(() => {
    ...
    // Fetch dashboard and inventory report sequentially to avoid overwhelming the server
