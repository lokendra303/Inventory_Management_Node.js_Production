...
const NewVendor = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await apiService.post('/vendors', apiData);
      ...
      if (response.success || response.data?.success) {
        ...
        message.error(response.error || 'Failed to create vendor');
      }
    } catch (error) {
      console.error('Error saving vendor:', error);
      const errorMsg = error.response?.data?.error || error.message || 'Failed to save vendor';
        ...
        <Card style={{ marginBottom: '24px', borderRadius: '4px' }}>
              ...
              <Form.Item
                label={
                  <span>
                    ...
                    <Tooltip title="How vendor will be displayed in the system">
                      <InfoCircleOutlined style={{ marginLeft: '8px', color: '#999' }} />
                    </Tooltip>
                  </span>
                }
            ...
            <Col xs={24} md={12}>
              <Form.Item 
                label={
                  <span>
                    Email Address
                    <Tooltip title="Primary email for vendor communication">
                      <InfoCircleOutlined style={{ marginLeft: '8px', color: '#999' }} />
                    </Tooltip>
                  </span>
                } 
          ...
          <Row gutter={[16, 16]}>
            <Col xs={24} sm={12} md={6}>
              <Form.Item label="Work Phone Code" name="workPhoneCode">
                <Select placeholder="+91" value="+91" options={[{ label: '+91', value: '+91' }]} />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} sm={12} md={3}>
              <Form.Item label="Mobile Code" name="mobileCode">
                <Select placeholder="+91" value="+91" options={[{ label: '+91', value: '+91' }]} />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={6}>
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              ...
              children: (
                <Card style={{ marginTop: '0px', borderTop: 'none' }}>
                  <p style={{ color: '#999', marginBottom: '16px' }}>No contact persons added yet.</p>
                  <Button type="primary">+ Add Contact Person</Button>
                </Card>
              )
            },
            {
            ...
            },
            {
              ...
              children: (
                <Card style={{ marginTop: '0px', borderTop: 'none' }}>
                  <p style={{ color: '#999' }}>No custom fields configured.</p>
                </Card>
              )
            },
            {
              ...
              children: (
                <Card style={{ marginTop: '0px', borderTop: 'none' }}>
                  <p style={{ color: '#999' }}>No reporting tags added.</p>
                </Card>
              )
            },
            {
